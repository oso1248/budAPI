...
@router.get('/read/sum/{inv_uuid}', status_code=status.HTTP_200_OK, response_model=List[val_inv_material.InvSumOut])
...
# Get Material Inventory By UUID Summed
@router.get('/read/complete/{inv_uuid}', status_code=status.HTTP_200_OK, response_model=List[val_inv_material.InvCompleteOut])
def get_inv_by_uuid_summed(inv_uuid: str, current_user: val_user.UserOut = Depends(get_current_user)):
    print(inv_uuid)
    cursor.execute("""
...
@router.get('/dates', status_code=status.HTTP_200_OK, response_model=List[val_inv_material.InvDatesOut])
